...
klassen de juiste namen geeft. Kijk daarom goed in [Methodes.java](srcethodes.java), [Main.java](srcain.java) en [PokemonGymImpl.java](srcokemonGymImpl.java). 
Daar worden alle klassenamen gebruikt.
Door de methodes in [Main](srcain.java) en [PokemonGymImpl](srcokemonGymImpl.java) 
...
We hebben bijna alle methodes al voor je in [Methodes.java](srcethodes.java) gezet, behalve getters en setters.
De methodes die hier niet staan, waaronder constructors, kun je afleiden uit [Main](srcain.java) en [PokemonGymImpl](srcokemonGymImpl.java).
We hebben een aantal variabelen die je nodig hebt, beschreven in [Variabelen.java](srcariabelen.java), 
maar je kunt ze ook afleiden uit de getters, setters en constructors die in [Main](srcain.java) en [PokemonGymImpl](srcokemonGymImpl.java) worden aangeroepen.
Het enige wat je nog moet doen is de methodes en variabelen in de juiste klasse(n) zetten. 
De variabelen uit [Variabelen.java](srcariabelen.java) moet je meestal nog een type geven en instantiëren, maar bij sommigen hebben we dat ook al gedaan.
...
1. Loop door [Main](srcain.java) en [PokemonGymImpl](srcokemonGymImpl.java) en los alle foutmeldingen op door klassens, variabelen en methodes aan te maken en af en toe iets te importeren.
2. Kijk in [Variabelen.java](srcariabelen.java) of daar nog niet-geimplementeerde variabelen staan. Plaats deze in de juiste klassen, die je zojuist hebt aangemaakt en geef ze de juiste datatypen.
3. Geef alle variabelen getters en setters als ze in de juiste klasse staan.
4. Doe hetzelfde voor [Methodes.java](srcethodes.java).
